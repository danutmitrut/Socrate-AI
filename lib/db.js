...
export async function initDatabase() {
  try {
    ...
    await sql`CREATE INDEX IF NOT EXISTS idx_stripe_events_event_id ON stripe_events(stripe_event_id)`;
...
export async function createUser(email, passwordHash, ipAddress) {
  ...
  const result = await sql`
    INSERT INTO users (email, password_hash, messages_limit, account_status)
    VALUES (${email}, ${passwordHash}, ${messagesLimit}, ${accountStatus})
    RETURNING *
  `;
...
export async function checkUserLimit(userId) {
  ...
  // Check if account is dormant (highest priority check)
...
/**
 * Set subscription cancel_at date
 */
export async function setSubscriptionCancelAt(userId, cancelAt) {
  const result = await sql`
    UPDATE users
    SET subscription_cancel_at = ${cancelAt}
    WHERE id = ${userId}
    RETURNING *
  `;
  return result.rows[0];
...
/**
 * Clear subscription cancel_at (when subscription is deleted or reactivated)
 */
export async function clearSubscriptionCancelAt(userId) {
  const result = await sql`
    UPDATE users
...
/**
 * Set account to active status (when user upgrades)
 */
 ...
 */
export async function isStripeEventProcessed(stripeEventId) {
  const result = await sql`
...
/**
 * Record Stripe event as processed
 */
export async function recordStripeEvent(stripeEventId, eventType, userId = null, metadata = null) {
  try {
    await sql`
      INSERT INTO stripe_events (stripe_event_id, event_type, user_id, metadata)
      VALUES (${stripeEventId}, ${eventType}, ${userId}, ${metadata ? JSON.stringify(metadata) : null})
    `;
    return true;
  } catch (error) {
